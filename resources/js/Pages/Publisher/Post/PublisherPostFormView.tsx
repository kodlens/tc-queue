...
import { UploadOutlined, ProjectOutlined, SaveOutlined, ArrowLeftOutlined } from "@ant-design/icons";
...
export default function AuthorPostCreateEdit({
    ...
    const uploadProps: UploadProps = {
        ...
        onRemove(info) {
            if (id > 0) {
                ...
                axios
                    .post(`/author/image-remove/${id}/${info.name}`)
                    .then((res) => {
    ...
    const submit = async (values: object) => {
        ...
        if (id > 0) {
            try {
                const res = await axios.patch("/publisher/posts/" + id, values);
